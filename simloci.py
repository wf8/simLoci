...
## TODO: allow input tree file, fix migrate, 
## print tre to file? deep geneflow, treemix output
...
def usage():
    ...
    print "\t -o <str>   output file prefix name"
    ...
    #print "\t -pt        prints tree info to screen (requires ete2 module)"
...
def simdata(params):
    ...
    ## CAN I ADD MIGRATION BETWEEN ANCESTRAL BRANCHES???
    if params["migs"]:
...
def makeloci(aligns, names, nloci, outname):
    ...
    for locus in range(nloci):
        ...
        for dicname in dicnames:
            ...
            print >>outloci, ">"+dicname+" "*(10-len(dicname))+\
                                           "".join(seqstring[-1])
                        ##makehetero(ldict[n][locus][0],ldict[n][locus][1])
        ## print snpstring to final line of locus
        print >>outloci, "//"+snpstring(np.array(seqstring), locus) 
    outloci.close()
...
def makestructure(outname, names):
    ...
    print outname, names, "\nstr output not implemented yet"
...
if __name__ == "__main__":
    ...
    makeloci(ALIGNS, NAMES, PARAMS["loci"], PARAMS["outname"])
